// content.js
// ======== ML code kept exactly as provided by you (only minimal messaging added) ========
...
// ====== new: send to background which will check OpenPhish first ======
chrome.runtime.sendMessage({
    ...
    if (response.source === "openphish") {
        alert("⚠️ This URL is listed in OpenPhish as phishing!");
    } else if (response.source === "ml") {
        var localPred = predict(response.features);
        if (localPred === 1) {
            alert("⚠️ Suspicious site detected by ML.");
        } else {
            console.log("✅ Site looks safe (ML).");
        }
    }
});